...
// - syncs with Mollusk proposal queue to mint shares for grantees
...
contract MolluskPool {
    ...
    uint256 public currentProposalIndex = 0; // the mollusk proposal index that this pool has been synced to

    Mollusk public mollusk; // mollusk contract reference
    IERC20 public approvedToken; // approved token contract reference (copied from mollusk contract)
    ...
    modifier active {
        require(totalPoolShares > 0, "MolluskPool: Not active");
        _;
    }

    modifier noReentrancy() {
        require(!locked, "MolluskPool: Reentrant call");
        locked = true;
        _;
        locked = false;
    }

    constructor(address _mollusk) public {
        mollusk = Mollusk(_mollusk);
        approvedToken = IERC20(mollusk.approvedToken());
    }

    function activate(uint256 initialTokens, uint256 initialPoolShares) public noReentrancy {
        require(totalPoolShares == 0, "MolluskPool: Already active");

        require(
            approvedToken.transferFrom(msg.sender, address(this), initialTokens),
            "MolluskPool: Initial tokens transfer failed"
        );
        _mintSharesForAddress(initialPoolShares, msg.sender);
    }

    // updates Pool state based on Mollusk proposal queue
    ...
    //   - protects pool contributors if many Mollusk members ragequit before the proposal is processed by reducing follow on funding
    //   - e.g. if 50% of Mollusk shares ragequit after someone voted yes, the grant proposal would get 50% less follow-on from the pool
    function sync(uint256 toIndex) public active noReentrancy {
        require(
            toIndex <= mollusk.getProposalQueueLength(),
            "MolluskPool: Proposal index too high"
        );
        ...
        while (i < toIndex) {
            ...
            (, applicant, sharesRequested, , , , processed, didPass, aborted, tokenTribute, , maxTotalSharesAtYesVote) = mollusk.proposalQueue(i);
            ...
            if (!aborted && didPass && tokenTribute == 0 && sharesRequested > 0) {
                ...
                //   2. sharesRequested is <= 10**18 (see Mollusk.sol:172), and
    ...
    function deposit(uint256 tokenAmount) public active noReentrancy {
        ...
        require(
            ...
            "MolluskPool: Deposit transfer failed"
        );
    ...
    function keeperWithdraw(uint256 sharesToBurn, address recipient) public active noReentrancy {
        require(
            ...
            "MolluskPool: Sender is not a keeper"
        );
    ...
    function _mintSharesForAddress(uint256 sharesToMint, address recipient) internal {
        ...
        require(
            ...
            "MolluskPool: Max number of shares exceeded"
        );
    ...
    function _withdraw(address recipient, uint256 sharesToBurn) internal {
        ...
        require(
            ...
            "MolluskPool: Not enough shares to burn"
        );
        ...
        require(
            ...
            "MolluskPool: Withdrawal transfer failed"
        );
